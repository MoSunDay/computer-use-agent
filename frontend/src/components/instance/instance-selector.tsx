...
export const InstanceSelector: FC<InstanceSelectorProps> = ({
  ...
  const getDeviceName = (sandbox: typeof sandboxList[0]) => {
    const osName = sandbox.OsType === "Darwin" ? "macOS" :
                   sandbox.OsType === "Windows" ? "Windows" :
                   sandbox.OsType === "Linux" ? "Linux" : sandbox.OsType;
    return `本地设备 (${osName})`;
  };
