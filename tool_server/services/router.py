...
async def handle(
    ...
    if version == "2020-04-01":
        ...
        # 过滤掉截图的 base64 数据，避免日志过长
        if action == "TakeScreenshot" and isinstance(result, dict) and "Screenshot" in result:
            logger.info("handle result: Screenshot taken (base64 data omitted, length: %d)", len(result.get("Screenshot", "")))
        else:
