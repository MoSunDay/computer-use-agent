...
class LocalSandboxManager(SandboxManager):
    """
    Local sandbox manager implementation for running without remote ECS
    Uses local tool server endpoint directly
    """
    _instance = None

    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(LocalSandboxManager, cls).__new__(cls)
    ...
    def get_tool_server_endpoint(self, sandbox_id: str=None) -> str | None:
        """
        Get local tool server endpoint URL

        Args:
            sandbox_id: Not used for local mode, kept for compatibility
        Returns:
            Local tool server URL
        """
        # For local mode, always return the local endpoint
        # The format should already be "http://127.0.0.1:8102" in config
        return self.tool_server_endpoint_format
