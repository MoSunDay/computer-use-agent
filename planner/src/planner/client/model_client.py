...
from openai.types.chat import ChatCompletionContentPartTextParam as ContentPartText
...
class ChatModelClient(object):
    ...
    def process_screenshot_and_update_history_messages(self, screenshot_image_url: str) -> str:
        # 添加文本内容以满足 Volcengine API 要求
        text_content = ContentPartText(type="text", text="这是当前屏幕截图，请分析并执行任务。")
        snap_content = ContentPartImage(type="image_url", image_url=ImageURL(url=screenshot_image_url))
        screenshot_message = UserMessage(role="user", content=[text_content, snap_content])
        self.messages.append(screenshot_message)
        self._remove_overflow_image_messages()
        # 调试：打印发送的 messages
        self.logger.debug("Sending messages to API: %s", self.messages)
    ...
    def _remove_overflow_image_messages(self):
        ...
        for msg in reversed(other_messages):
            ...
            if isinstance(cs, list):
                # 统计图片数量
                image_count = len([c for c in cs if c.get("type") == "image_url"])
